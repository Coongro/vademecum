...
- **Trigger name**: `trg_vademecum_{descriptive_name}`
- **Function name**: `trg_vademecum_{descriptive_name}_fn`

Where `vademecum` is your plugin's moduleId (pluginId with hyphens replaced by underscores).
...
CREATE OR REPLACE FUNCTION "trg_vademecum_audit_log_fn"()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER "trg_vademecum_audit_log"
AFTER INSERT OR UPDATE ON "module_vademecum_items"
FOR EACH ROW
EXECUTE FUNCTION "trg_vademecum_audit_log_fn"();
```

> **Important**: The system automatically **DISABLE**s all `trg_vademecum_*` triggers when the plugin
